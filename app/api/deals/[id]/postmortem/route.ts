...
export async function POST(
  ...
  const dealContext = `Client: ${deal.client_name ?? "unknown"} at ${deal.client_company ?? "unknown"}. Pain: ${deal.pain_point ?? "n/a"}. Budget signal: ${deal.budget_signal ?? "n/a"}. Timeline: ${deal.timeline ?? "n/a"}. Proposed amount: ${deal.proposed_amount ?? "n/a"}. Notes: ${await notesForPrompt(supabase, id, deal.notes, "private")}.`;
      ...
      try {
        const response = anthropic.messages.stream({
          ...
          messages: [
            {
              role: "user",
              content: postmortemPrompt(dealContext, deal.transcript ?? ""),
            },
          ],
        });
